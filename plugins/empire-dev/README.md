...
Spawn parallel specialist subagents to review a diff or PR, then aggregate findings into a tiered, verified report (Consensus / Corroborated / Single-source). The skill detects signals in the diff (language, security surface, architectural change, perf hotspots, tests, structural growth), picks 3–4 specialists from the available roster (weaker signals fold into broader briefs), dispatches them in parallel, then votes findings by `(file, line-range, category)` match. Reviewers are briefed to be ambitious rather than merely defect-hunting: each looks for the reframing that deletes complexity in its own domain — delete the attack surface, not guard it; remove the work, not cache it; make illegal states unrepresentable so branches disappear — while a restructuring proposal only counts if it removes more code and concepts than it adds, and any purely additive suggestion still has to cite the defect it fixes. Findings below the consensus threshold (strict majority and at least 3 specialists), nits aside, each get their own independent verifier agent — the bundled fast `finding-verifier` by default — dispatched in parallel (one per finding), ruling valid/invalid against a fixed severity rubric, blind to specialist count, so a real issue caught by only one specialist isn't buried just for being single-source. Built for speed: the diff, `CONTEXT.md` vocabulary, and relevant `docs/adr/` summaries are prepared once and inlined into every brief (verifiers get just their finding's hunk), so subagents skip redundant repo reads; prep overlaps any confirmation question. Runs via the bundled `team-review.js` workflow on any host with a JS workflow runner (Claude Code's `Workflow`, pi's `workflow` tool from the `pi-dynamic-workflows` extension) — structured specialist output, verifiers dispatched eagerly as each specialist returns (no cross-wave barrier), deterministic tier math in JS, specialists on a fast mid-tier model and verifiers on a cheap one, and every agent given a generous per-agent timeout plus a stated grace period in its brief so it paces itself and returns partial findings instead of timing out empty — with a documented inline-Agent fallback. Findings stay local — never posted to GitHub.
...
| `simplifier`           | Structural quality: file sprawl, spaghetti growth, wrappers, layer leaks |
