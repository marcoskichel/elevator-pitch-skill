...
# Session id source: $CLAUDE_CODE_SESSION_ID, then $AGENT_SESSION_ID (falls back
# to "unknown" with warning).
#
# Subcommands:
...
session_id() {
  local sid="${CLAUDE_CODE_SESSION_ID:-${AGENT_SESSION_ID:-}}"
  if [[ -n "$sid" ]]; then
    printf '%s' "$sid"
  else
    warn "no agent session id (CLAUDE_CODE_SESSION_ID or AGENT_SESSION_ID) set — using 'unknown' namespace (best-effort, not authoritative)"
    printf '%s' 'unknown'
  fi
}
